...
public class ProductCommandController {
    ...
    @PostMapping
    public ResponseEntity<Product> create(@Valid @RequestBody ProductCreateCommand cmd) {
        Product product = productCommandService.createProduct(cmd);
        return ResponseEntity.status(HttpStatus.CREATED).body(product);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Product> update(@PathVariable Long id, @Valid @RequestBody ProductUpdateCommand cmd) {
        cmd.setId(id);
